...
export class BrandProductsNewComponent implements OnInit {
  ...
  accept() {
    ...
    const ProductionLine = this.productLines.filter(
      (e) => e.groupLineId === this.f.ProductionLine.value
    )[0];
    const TimeUnit = this.DaysWeeks.filter(
      (e) => e.timeUnitId === this.f.DaysWeeks.value
    )[0];
    const ExFactoryLocation = this.ExFactoryLocations.filter(
      (e) => e.factoryLocationId === this.f.ExFactoryLocation.value
    )[0];
    const Status = this.Status.filter(
      (e) => e.brandCatalogStatusId === this.f.Status.value
    )[0];
    const brandCustomer = this.brandCustomers.filter(
      (e) => e.masterCustomerId === this.f.BrandCustomer.value
    )[0];
  ...
  updateFileList(isMain: boolean, isDesigner: boolean) {
    ...
    if (isMain) {
      this.f.UploadArtFile.setValue(filesUpload);
    } else {
      this.f.UploadAditional.setValue(filesUpload);
    }
  }
  ...
  createBrandProduct() {
    this.submitted = true;
    const brandProduct = {
      masterCustomerId: this.f.BrandCustomer.value,
      brandCatalogCode: this.f.CustomerCode.value,
      brandCatalogName: this.f.Description.value,
      groupLineId: this.f.ProductionLine.value,
      latestVersion: this.fixDate(this.f.LastestVersion.value),
      dimension: this.f.Dimentions.value,
      finalDimension: this.f.FinalDimentions.value,
      moq: this.f.MinimunOrderQuantity.value,
      minimunPerChange: this.f.MinumumPerSku.value,
      leadTime: this.f.LeadTime.value,
      leadTimeUnit: this.f.DaysWeeks.value,
      factoryLocationId: this.f.ExFactoryLocation.value,
      localFacility: this.f.ProductionFacilites.value,
      brandCatalogStatusId: this.f.Status.value,
      approvedBy: this.f.ApprovedBy.value,
      approvedDate: this.f.ApprovedDate.value
        ? this.fixDate(this.f.ApprovedDate.value)
        : null,
      constructionDetail: this.f.ConstructionDetail.value,
  ...
  public resetFiles(isMain: string) {
    if (isMain == 'file') {
      this.f.UploadArtFile.setValue('');
    } else {
      this.f.UploadAditional.setValue('');
    }
    ...
    if (event.target.value.trim().toUpperCase() != '') {
      ...
      if (!this.addBrandProductForm.valid) {
        ...
        const ProductionLine = this.ListDetails.filter(
          (e) =>
            e.productId.trim().toUpperCase() === event.target.value.trim().toUpperCase() &&
