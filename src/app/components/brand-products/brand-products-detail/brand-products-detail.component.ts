...
export class BrandProductsDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.product.latestVersion = moment(this.product.latestVersion).format('MMM/DD/YY').replace('.','').toUpperCase();
    this.product.approvedDate = moment(this.product.approvedDate).format('MMM/DD/YY').replace('.','').toUpperCase();
    this.translate
