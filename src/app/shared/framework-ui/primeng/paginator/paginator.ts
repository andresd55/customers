...
import { SelectItem, SharedModule } from '../api/public_api';
...
@Component({
    ...
    template: `
        <div [class]="styleClass" class="row" [ngStyle]="style" [ngClass]="'p-paginator p-component'" *ngIf="(alwaysShow ? true : (pageLinks && pageLinks.length > 1)) && totalRecords">
            <div class="col-12 {{!columnsSeparate ? 'col-md-4' : ''}}">
            <p-dropdown [options]="rowsPerPageItems" [(ngModel)]="rows" *ngIf="rowsPerPageOptions" styleClass="p-paginator-rpp-options w-66 h-33" [disabled]="empty()"
            ...
            <label class="lblEntries">{{ 'general.lblEntriesPerPage' | translate }}</label>
            </div>
            <div class="col-12 {{!columnsSeparate ? 'col-md-3' : ''}} mt-2">
                <label class="lblEntries">{{ currentPageReport }}</label>
            </div>
            <div class="col-12 {{!columnsSeparate ? 'col-md-5' : ''}} mt-2 text-right p-0">
...
export class Paginator implements OnInit, OnChanges {
    ...
    onPageLinkClick(event: any, page: any) {
    ...
    onPageDropdownChange(event: any) {
    ...
    get currentPageReport() {
        this.translate.get('general.lblShowing').subscribe((data:any)=> {
