import {
    ...
    ContentChildren, QueryList, ChangeDetectorRef, Inject, forwardRef, TemplateRef, ViewRef, ChangeDetectionStrategy, ViewEncapsulation
} from '@angular/core';
import { trigger, state, style, transition, animate } from '@angular/animations';
import { CommonModule } from '@angular/common';
import { Subscription } from 'rxjs';
import { BlockableUI, Header, PrimeTemplate, SharedModule } from '../api/public_api';
import { TranslateLoader, TranslateModule, TranslateService } from '@ngx-translate/core';
import { httpTranslateLoader } from '../paginator/paginator';
                ...
                <a role="tab" class="p-accordion-header-link " (click)="toggle($event)" (keydown)="onKeydown($event)" [attr.tabindex]="disabled ? null : 0"
                    ...
                    <div class="row w-100">
                        <div class="col-9 d-table">
                            <span class="p-accordion-header-text align-middle" *ngIf="!hasHeaderFacet">
                                {{header}}
                            </span>
                        </div>

                        <div *ngIf="formStatus" class="col-3 text-left d-table">
                            <img src="assets/images/clientStatus{{imagenIcon}}.svg" class="img-status mr-2" alt="status image"/>
                            <label class="mt-1 label-status">{{ statusFormText | translate }}</label>
                        </div>
                    </div>
                   
                    <span class="p-accordion-toggle-icon position-icon-arrow" [ngClass]="selected ? accordion.collapseIcon : accordion.expandIcon"></span>
...
export class AccordionTab implements AfterContentInit, OnDestroy {
    ...
    @Input() imagenIcon: number = 1;

    @Input() statusFormText: string;
    ...
    constructor(@Inject(forwardRef(() => Accordion)) accordion, public changeDetector: ChangeDetectorRef, public translate: TranslateService,) {
        this.accordion = accordion as Accordion;
    }

    ngAfterContentInit() {

        console.log(this.statusFormText);
